	...
	return (
        ...
        <meta property="og:image" content={film.image} />
					...
					<FacebookButton
						url={
							process.env.NODE_ENV === "production"
								? location
								: "https://phim-pro.netlify.app/film/17"
						}
					...
					<div className="details-option">
						<p>
							...
							<span className="orange-text">
								{" "}
								{film.upComing === "true" ? "Chưa ra mắt" : "Hoàn tất"}
							</span>
						</p>
						<p>
							<span className="bold">Đạo diễn:</span>{" "}
							<span className="orange-text">{film.directors},</span>
						</p>
						<p>
							<span className="bold">Quốc gia:</span>{" "}
							<span className="orange-text">{film.country},</span>
						</p>
						<p>
							<span className="bold">Lượng yêu thích:</span>{" "}
							<span className="orange-text">{film.fav_count} người</span>
						</p>
						<p>
							<span className="bold">Thời lượng:</span>{" "}
							<span className="orange-text">
								{film.is_multi === "true" ? "45" : "90"} phút/tập
							</span>
						</p>
						<p>
							<span className="bold">Số tập:</span>{" "}
							<span className="orange-text">{film.episode} tập</span>
						</p>
						<p>
							<span className="bold">Chất lượng:</span>{" "}
							<span className="orange-text">Nhúng</span>
						</p>
						<p>
							<span className="bold">Số bình luận:</span>{" "}
							<span className="orange-text">
								{film.comment_count} bình luận
							</span>
						</p>
						<p>
							<span className="bold">Ngôn ngữ:</span>{" "}
							<span className="orange-text">Phụ đề Việt</span>
						</p>
						<p>
							<span className="bold">Thể loại:</span>{" "}
							<span className="orange-text">{film.category}</span>
						</p>
						{film.episode ? (
							<p>
			...
			<ListFilm
				type="row"
				films={films.filter(
					item => item.is_multi === film.is_multi && item.id !== film.id
				)}
			/>
		</section>
	);
