...
const Episodes = ({ episode, handleView }) => {
  const [showAddEp, setShowAddEp] = useState(false);
  const role = useSelector((state) => state.users.role);
  const [episodeData, setEpisodeData] = useState({ url: "", number: "", id: "", film_id: "" });
  ...
  return (
    <article className={`btn-episode`}>
      ...
      <span className="child" onClick={() => handleView(episode.url)}>
        {episode.number_ep}
      </span>
    </article>
  );
